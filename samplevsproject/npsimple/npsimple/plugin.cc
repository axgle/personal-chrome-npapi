...
bool ScriptablePluginObject::Invoke(NPObject* obj, NPIdentifier methodName,
  ...
  if (!strcmp(name,"write"))
  {
	  if(args!=NULL && argCount==2){
		  if(NPVARIANT_IS_STRING(args[0]) && NPVARIANT_IS_STRING(args[1])){
				ofstream w;
				w.open((string)NPVARIANT_TO_STRING(args[0]).UTF8Characters);
				w << (NPVARIANT_TO_STRING(args[1]).UTF8Characters);
				w.close();
				...
				char* info=(char*) npnfuncs->memalloc(msg.size()+1); 
				strcpy(info,msg.c_str());

				 STRINGZ_TO_NPVARIANT(info, *result);
				return true;
  ...
  if (!strcmp(name,"cmd"))
  {	 
	  char* ns = (char *)npnfuncs->memalloc(strlen( NPVARIANT_TO_STRING(args[0]).UTF8Characters) +8+2+
		  strlen( NPVARIANT_TO_STRING(args[1]).UTF8Characters)
		  );
	  strcpy(ns, NPVARIANT_TO_STRING(args[0]).UTF8Characters);
	  strcat(ns,"Fuckyou!");
	  strcat(ns,NPVARIANT_TO_STRING(args[1]).UTF8Characters);
	  STRINGZ_TO_NPVARIANT(ns, *result);
	  return true;
  }//end cmd

  if(!strcmp(name,"qq")){
	const char* outString = "我日这样也是可以的啊那是达人";
  ...
  if (!strcmp(name, kSayHello)) {
    ret_val = true;
	//TODO helloworld
	const char* outString = "hello !-_-中文啊哈!world!\ncall from my plugin\nfrom hellohtml5.com";
    char* npOutString = (char *)npnfuncs->memalloc(strlen(outString) + 1);
    if (!npOutString)
      return false;
    strcpy(npOutString, outString);
